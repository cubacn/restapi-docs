...
. Создайте нового {manual}/users.html[пользователя] с логином *promo-user*, от лица которого будет выполняться аутентификация по промо-коду.

. Назначьте созданную роль пользователю *promo-user*.
