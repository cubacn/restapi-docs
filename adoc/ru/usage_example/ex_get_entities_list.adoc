...
* *view* - {manual}/views.html[представление], с которым должны быть загружены сущности. В нашем примере представление `order-edit-view` содержит ссылку на `customer`.
...
Обратите внимание, что для каждой сущности загружаются атрибуты `_entityName` с именем сущности и `_instanceName`, содержащий результат вычисления {manual}/entity_class_annotations.html#namePattern_annotation[короткого имени] для сущности.
