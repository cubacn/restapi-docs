...
Файл `rest-queries.xml` должен находиться в главном пакете модуля *web* или *portal* (например, `com.company.myapp`). Его содержимое определяется схемой {xsd_url}/rest-queries.xsd[rest-queries.xsd], например:
...
Атрибут `cacheable` элемента `query` включает {manual}/entity_cache.html[кэширование] данного запроса.
